...
def rect_matrix_multiplication(l, d, cm1, cm2, sigma, tau, v, w, galois_keys, evaluator):
    cipher_result1 = []
    cipher_result2 = []
    ...
    # Step 4
    # ct.A*B <- ct.A_bar*B
    for k in range(int(np.log(d / l))):
        ct_rot = Ciphertext()
        evaluator.rotate_vector(cipher_mult, -l * d * 2 ^ k, galois_keys, ct_rot)
        evaluator.add_inplace(cipher_mult, ct_rot)
...
def matrix_mult_test(n=4):
    ...
    # ---------------------------------------------------------
    start = time.time()
    cipher_result = matrix_multiplication(n, cipher_matrix1, cipher_matrix2, plain_u_sigma_diagonals,
                                          plain_u_tau_diagonals, plain_v_k_diagonals, plain_w_k_diagonals, galois_keys,
                                          evaluator)
    end = time.time()

    # ---------------------------------------------------------
    plain = Plaintext()
    decryptor.decrypt(cipher_result, plain)
    vec = ckks_encoder.decode(plain)
    print(vec[:n ** 2].reshape(n, n))
    print('Mult Time: {:.3f}s'.format(end - start))

def rect_matrix_mult_test(l=3, d=6):
    parms = EncryptionParameters(scheme_type.CKKS)
    poly_modulus_degree = 16384
    parms.set_poly_modulus_degree(poly_modulus_degree)
    parms.set_coeff_modulus(CoeffModulus.Create(
    ...
    # matrix1 = np.random.rand(n, n)
    matrix1 = np.arange(1, l * d + 1).reshape(l, d)
    matrix1_bar = np.tile(matrix1, (int(d / l), 1))
    matrix2 = np.arange(1, d * d + 1).reshape(d, d)
    print("Expected:")
    print(np.dot(matrix1, matrix2))

    plain_matrix1 = ckks_encoder.encode(matrix1_bar.flatten(), scale)
    plain_matrix2 = ckks_encoder.encode(matrix2.flatten(), scale)
    cipher_matrix1 = encryptor.encrypt(plain_matrix1)
    cipher_matrix2 = encryptor.encrypt(plain_matrix2)
    ...
    # ---------------------------------------------------------
    start = time.time()
    cipher_result = rect_matrix_multiplication(l, d, cipher_matrix1, cipher_matrix2, plain_u_sigma_diagonals,
                                          plain_u_tau_diagonals, plain_v_k_diagonals, plain_w_k_diagonals, galois_keys,
                                          evaluator)
    end = time.time()
...
def matrix_transpose_test(n=4):
    ...
    # ---------------------------------------------------------
    p1 = Plaintext()
    decryptor.decrypt(cipher_result, p1)
...
if __name__ == "__main__":
    ...
    print("Rect Matrix Multiplication:")
    rect_matrix_mult_test()