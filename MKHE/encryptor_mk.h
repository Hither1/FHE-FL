//
// Created by avacado on 2020/11/25.
//
...
namespace seal
{
    ...
    */
    ...
    public:
        ...
        */
        Encryptor_MK(const SEALContext &context, const std::vector<PublicKey> &public_keys);
        ...
        */
        Encryptor_MK(const SEALContext &context, const SecretKey &secret_key);
        ...
        */
        Encryptor_MK(const SEALContext &context, const std::vector<PublicKey> & public_keys, const SecretKey &secret_key);
        ...
        @throws std::invalid_argument if public_key is not valid
        */
        inline void set_public_keys(const std::vector<PublicKey> &public_keys)
        {
            for(int i=0; i < public_keys.size(); i++)
            {
                if (!is_valid_for(public_keys[i], context_))
                {
    ...
    private:
        Encryptor_MK(const Encryptor_MK &copy) = delete;

        Encryptor_MK(Encryptor_MK &&source) = delete;

        Encryptor_MK &operator=(const Encryptor_MK &assign) = delete;

        Encryptor_MK &operator=(Encryptor_MK &&assign) = delete;
